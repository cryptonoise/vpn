...
if ! id -u suser &>/dev/null; then
    # Пользователь отсутствует — создаём и запрашиваем пароль
    ...
    while true; do
        ...
        if echo "suser:$password" | chpasswd; then
            ...
            printf "❌  Не удалось установить пароль. Используйте минимум 8 символов (буквы, цифры).\n"
        fi
    done
else
    # Пользователь уже существует — просто сообщаем и идём дальше
