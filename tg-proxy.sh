...
check_root() {
    if [[ $EUID -ne 0 ]]; then
        echo "❌ Скрипт требует прав root. Запустите с sudo." >&2
        exit 1
    fi
}
...
setup_firewall() {
    ...
    echo "✅ Фаервол настроен (порт $PROXY_PORT открыт)"
}

# Запрос параметров у пользователя
ask_params() {
    echo ""
    echo "⚙️ Настройка прокси"
    echo ""
    
    read -rp "🔹 Введите порт для прокси [8443]: " PROXY_PORT
    PROXY_PORT=${PROXY_PORT:-8443}
    if ! [[ "$PROXY_PORT" =~ ^[0-9]+$ ]] || [ "$PROXY_PORT" -lt 1 ] || [ "$PROXY_PORT" -gt 65535 ]; then
        echo "⚠️ Некорректный порт, используем 8443"
        PROXY_PORT=8443
    fi
    ...
    if [[ -z "$PROXY_DOMAIN" ]]; then
        ...
        echo "ℹ️ Будет использован IP: $PROXY_DOMAIN"
    else
...
generate_secret() {
    ...
    echo "✅ Секрет сгенерирован"
}

# Запуск контейнера
run_proxy() {
    echo "🚀 Запуск MTProxy контейнера..."
    docker run -d \
        --name telegram \
...
save_secret() {
    ...
    echo "ℹ️ Секрет сохранён в ~/mtproxy_secret.txt"
}
